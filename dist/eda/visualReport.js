...
exports.generateVisualReport = generateVisualReport;
...
function topTeamsByMatches(rows, limit = 8) {
...
function buildFormSeries(rows, teams) {
    const series = {};
...
function buildDinamoSummary(rows, teamName = "Dinamo Minsk") {
...
function findDinamoInStandings(standings) {
    if (!standings?.length) {
        return undefined;
    }
...
function buildDinamoGoalTimeline(rows, teamName = "Dinamo Minsk") {
    const teamRows = rows.filter((r) => r.homeTeam === teamName || r.awayTeam === teamName);
    const labels = [];
    const goalsFor = [];
    const goalsAgainst = [];
...
<body>
  ...
  <script>
    ...
    await (0, fs_1.ensureDirForFile)(targetFile);
    await (0, promises_1.writeFile)(targetFile, html, "utf-8");
    return targetFile;
}
